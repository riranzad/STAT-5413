...
X_plot <- make_kernel_basis(coords_plot, knots, kernel = "gaussian", bandwith = 10)## plot the kernel basis
dat_plot <- data.frame(
    x     = coords_plot[, 1], 
    y     = coords_plot[, 2],
    X     = c(X_plot),
...
for (j in 1:8) {
    ...
    X <- make_kernel_basis(coords, knots, kernel = "gaussian", bandwith = 10)
...
colnames(pred_coords) <- c("lon", "lat")
X_pred <- make_kernel_basis(pred_coords, knots, kernel = "gaussian", bandwith = 10)
...
Notice that the above model is overfit -- see the large variance surface. This can be resolved with penalized models (ridge, lasso, elastic net, etc).
...
curve(wendland_basis(abs(x - 0.5), radius = 0.15), n = 1000)
