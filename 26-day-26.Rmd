...
\begin{align*}
\boldsymbol{\Sigma}_p \otimes \mathbf{R}_n & \mathbf{L}\mathbf{L}' \\
& = \mathbf{L}_n \otimes \mathbf{L}_p
\end{align*}
...
\begin{align*}
\mathbf{y}(\mathbf{s}) = \begin{pmatrix} y_1(\mathbf{s}) \\ \vdots \\ y_p(\mathbf{s}) \sim N(\mathbf{A} \mathbf{f}(\mathbf{s}), \mathbf{D}) \end{pmatrix},
\end{align*}
...
- Seperable model: If the latent processes $f_i$ are $iid$ GPs with mean zero, variance 1 and correlation $C(\|\mathbf{s} - \mathbf{s}'\| | \boldsymbol{\theta})$, then the covariance is
...
0 & 0 & \cdots & Cov(f_L(\mathbf{s}), f_L(\mathbf{s})) \end{pmatrix}
 \mathbf{A}' \\
& \mathbf{A} \mathbf{A}' C(\|\mathbf{s} - \mathbf{s}'\|),
\end{align*}

where $\mathbf{A} \mathbf{A}'$ is the cross-covariance and $C(\|\mathbf{s} - \mathbf{s}'\|)$ is the spatial covariance
...
- Non-seperable model: If the latent processes $f_i$ are $iid$ GPs with mean zero, variance 1 but the latent process $j$ has correlation function $C(\|\mathbf{s} - \mathbf{s}'\| | \boldsymbol{\theta}_j)$, then the covariance is
